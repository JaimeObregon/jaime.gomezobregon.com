...
He aquí el código fuente completo y los contenidos de [mi sitio web personal](https://jaime.gomezobregon.com). Lo inicié en el verano de 2006, entonces como un blog, y desde 2020 reconstruido a medida —manteniendo todos sus contenidos— con énfasis en **el minimalismo y la eficiencia**.
...
Es una aplicación estática (_frontend_) JavaScript, desarrollada íntegramente sobre estándares web (HTML, CSS, JavaScript, Web Components…), sin _frameworks_ ni librerías externas. Tampoco hay ningún lenguaje de _scripting_ en el lado del servidor, ni se requiere de una base de datos.
...
El proyecto se construye y despliega en Netlify con cada _push_ a la rama `master`.
...
1. **Soporte para entradas programadas**. Simplemente definiendo una fecha futura de publicación en la propiedad `date_published` de la entrada en [`index.json`](/httpdocs/index.json). El artículo no aparecerá en la portada hasta dicha fecha. Véase [#8](https://github.com/jaimeobregon/jaime.gomezobregon.com/issues/8) para los detalles.

1. **Entradas destacadas**. Puedes destacar artículos en la portada sin más que añadir `Destacado` a la propiedad `tags`. Más información en [#7](https://github.com/jaimeobregon/jaime.gomezobregon.com/issues/7).
...
1. **Maquetación para impresora**. He incorporado una hoja de estilos específica para mejorar la experiencia de quien decida imprimir un contenido. La reflexión está en [#3](https://github.com/jaimeobregon/jaime.gomezobregon.com/issues/3).

1. **Notas al pie**. Son muy útiles para proporcionar referencias o añadir aclaraciones marginales sin entorpecer el desarrollo narrativo del texto. Véase [esta entrada con notas al pie](https://jaime.gomezobregon.com/del-limitado-acceso-a-las-resoluciones-judiciales) para un ejemplo de uso.
...
La deliberada austeridad que persigo en el diseño tiene una excepción consciente en la filigrana del separador temático y las floridas letras miniadas (también llamadas _letras capituales_ o _iniciales_).

Para estas últimas he preferido las diseñadas por [William Morris](https://en.wikipedia.org/wiki/William_Morris) en el s. xɪx, que he encontrado en [Openclipart](https://openclipart.org/search/?query=William+Morris+Letter) bajo licencia de dominio público. Por una cuestión de rendimiento en el sitio las incrusto como elementos SVG. He optimizado el trazado vectorial de cada una de estas 26 letras capitulares, que comparto en [`/httpdocs/initials`](/httpdocs/initials) para quien las desee.

# El manifiesto: un tributo a las cosas sencillas

En 2003 presenté mi Proyecto Fin de Carrera, sobre **sistemas de gestión de contenidos** (CMS, por sus siglas en inglés). En estos veinte años he trabajado con cientos de implantaciones de decenas de CMS diferentes. En este proyecto he querido dotarme del _anti-CMS_.

**El abuso de la publicidad invasiva, el SEO agresivo y la economía de la atención están destruyendo la web**: los autores ya no escriben artículos para sus lectores, sino para _generar tráfico_. Y los técnicos ya no escriben código para los navegadores de sus usuarios, sino para _posicionarse arriba en buscadores_. Es [una perversión de los principios](https://neustadt.fr/essays/against-a-user-hostile-web).

Yo aquí me apeo de ese mundo. Considero que hace falta **un reinicio, una inversión radical de valores**: volver a poner la humanidad y la sensibilidad en el centro y regresar a la esencia desnuda de las cosas.
...
- **Excelencia técnica.** Este sitio web utiliza exclusivamente tecnologías modernas y estándar: HTML, CSS y JavaScript. Aquí escribo marcado semántico, código válido conforme a las especificaciones, y proporciono metadatos a los contenidos. No utilizo más clases (`class`) ni identificadores (`id`) que los imprescindibles.

- **Muerte a las _cookies_ prescindibles.** La ley europea obliga a informar al usuario antes de instalar _cookies_ de seguimiento en su navegador. Sin embargo, la gran mayoría de los sitios web **en realidad no necesitarían instalar ninguna cookie**. Pero lo excepcional se ha convertido en norma y ahora hasta los sitios más anodinos de la web requieren docenas de _cookies_ y **molestan al usuario** pidiendo su consentimiento para instalarlas. Este es mi sitio web, y no instalará ninguna _cookie_ de seguimiento en tu navegador. Por lo tanto, tampoco te molestará pidiéndote consentimiento.
...
Debes [contactarme](https://jaime.gomezobregon.com) antes de reutilizar o republicar mi contenido bajo otras condiciones.
